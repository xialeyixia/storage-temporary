  ...
  <div class="tree">
    ...
    >
      <template #default="{ data }">
        <div
          ...
          :style="!data.ip ? 'width: 100%' : ''"
          :title="
            data.ip
              ? $t('common.status') +
                '：' +
                (data.state === 0 ? $t('common.online') : $t('common.offline'))
              : null
          "
        >
          <span class="node-icon">
            ...
            <svg
              t="1630545147960"
              v-if="data.state === 0"
              style="margin: -2px -5px -2px -11px; vertical-align: bottom"
              viewBox="0 0 1024 1024"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              p-id="6629"
            ...
            >
              ...
              ></path>
              <path
                d="M739.84 371.2c-20.48-20.48-52.48-20.48-72.96 0L467.2 572.16l-110.08-110.08c-20.48-20.48-52.48-20.48-72.96 0-20.48 20.48-20.48 52.48 0 72.96L428.8 678.4c10.24 10.24 24.32 15.36 37.12 15.36 14.08 0 26.88-3.84 37.12-15.36l235.52-235.52c21.76-19.2 21.76-52.48 1.28-71.68z"
                fill="#ffffff"
                p-id="6631"
          ...
          <el-button
            ...
            size="small"
            @click="systemConfig"
            >{{ $t('control.configure') + $t('common.node') }}</el-button
          >
        </div>
      </template>
    </el-tree>
    <re-dialog v-model="sysConfVisible" :width="300">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button
            ...
            type="primary"
            @click="addNode"
            :disabled="sysConfForm.nodeConf.length > 2"
            >{{ $t('control.add') + $t('common.node') }}</el-button
          >
          <el-button
            size="small"
            @click="sysConfVisible = false"
      ...
      >
        ...
        <div
          v-for="(node, index) in sysConfForm.nodeConf"
          :key="index"
          style="width: 100%"
        >
          <el-form-item
          ...
          >
            ...
            ></el-input>
            <el-button @click.prevent="removeNode(index)" type="primary">{{
              $t('control.delete')
            }}</el-button>
          </el-form-item>
        </div>
    ...
    sureConfig() {
      ...
      this.$refs.sysConfForm.validate((valid) => {
        if (valid) {
          ...
          this.$api
            .post('/webapp/set_clusterconfig', querydata, true)
            .then((res) => {
