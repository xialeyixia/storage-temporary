<template>
  <div class="storage-content">
    ...
    <el-button
      ...
      type="primary"
      size="small"
      @click="createNFSClient"
      >{{ $t('storage.mountclient') }}</el-button
    >
        ...
        <el-table-column :label="$t('table.operate')" fixed="right">
          <template #default="scope">
            <el-button
              size="small"
              type="primary"
              @click="removeNFServer(scope.row)"
    ...
    <re-dialog v-model="serverVisible" :width="500">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            type="primary"
            size="small"
            @click="sureaddNFSserver"
            :disabled="sureLoading || !serverForm.path"
            :loading="sureLoading"
      ...
      <div style="width: 100%">
        ...
        <div style="margin-bottom: 10px">
          {{ $t('common.choosedevice') + ' :' }}
        </div>
        <el-table
          v-loading="tableLoading"
          element-loading-text="Loading..."
          :data="tableFilter(fileSystemData)"
    ...
    <re-dialog v-model="clientVisible" :width="500">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            type="primary"
            size="small"
            @click="sureMountNFSClient"
            :disabled="sureLoading || !clientForm.path"
            :loading="sureLoading"
            >{{ $t('common.confirm') }}</el-button
          >
        </span>
      ...
      >
        ...
        <span
          v-show="clientForm.protocol == 'SMB'"
          style="
            margin-bottom: 10px;
            color: #c53838;
            margin-top: -18px;
            ...
            width: 100%;
          "
          >* 若服务端未配置(map to guest = Bad User;)，则只能使用 Samba
          用户访问，不允许匿名访问。</span
        >
        <el-form-item
          :label="$t('storage.mounton')"
        ...
        <el-form-item label="IP" style="width: 100%" prop="ip">
          <el-input v-model="clientForm.ip" :maxlength="30">
            <template #append>
              <el-button :loading="tableLoading" @click="onSearchNFSClient"
...
export default {
  ...
  methods: {
    ...
    getNFSserverlist() {
      ...
      this.$api
        .get(
          '/webapp/get_server_nas?remote_ip=' + this.$route.query.ip,
          {},
          true,
        )
        .then((res) => {
    ...
    sureaddNFSserver() {
      ...
      this.$api
        .post(
          '/webapp/set_server_nas?remote_ip=' + this.$route.query.ip,
          querydata,
          true,
        )
        .then((res) => {
    ...
    removeNFServer(row) {
      this.$messageBox
        .confirm(
          this.$t('control.delete') +
            ' NFS ' +
            this.$t('storage.server') +
            ' - ' +
            row.path,
        ...
        .then(() => {
          ...
          this.$api
            .deletes(
              '/webapp/delete_server_nas?remote_ip=' + this.$route.query.ip,
              row,
              true,
            )
            .then((res) => {
    ...
    onSearchNFSClient() {
      this.$refs.clientForm.validate((valid) => {
        if (valid) {
          ...
          this.$api
            .get(
              '/webapp/get_remote_nas?remote_ip=' + this.$route.query.ip,
              querydata,
              true,
            )
            .then((res) => {
    ...
    sureMountNFSClient() {
      this.$refs.clientForm.validate((valid) => {
        if (valid) {
          ...
          this.$api
            .post(
              '/webapp/set_client_nas?remote_ip=' + this.$route.query.ip,
              querydata,
              true,
            )
            .then((res) => {
