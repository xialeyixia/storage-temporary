<template>
  <div class="storage-content">
    <div class="btn">
      <el-button color="#579cdc" style="color: #fff;" type="primary" size="small" @click="saveConfig">{{ $t('common.save') }}</el-button>
      <div>
        ...
        <el-button color="#579cdc" style="color: #fff;" type="primary" size="small" @click="onShowProver">{{ $t('common.prover') }}</el-button>
      </div>
    </div>
    <el-form class="system-form" label-suffix="：" label-width="115px" label-position="left">
      ...
      <el-form-item :label="$t('storage.maxnumoflogs')">
        <el-input-number v-model="systemInfo.maxLogNum" :controls="false" style="width: 100%; text-align: left" :max=1000000000></el-input-number>
      </el-form-item>
      ...
      <el-form-item :label="'HA' + $t('storage.partnernode') + 'IP'" v-if="systemInfo.UseHAMode != 0">
        <el-select v-model="systemInfo.HAPartnerIP"  @visible-change="getNodesConfig" placeholder=" " clearable>
          <el-option v-for="(val, index) in nodesData" :key="index" :label="val.ip" :value="val.ip"></el-option>
        </el-select>
      </el-form-item>
    ...
    <re-dialog v-model="showChangePwd" :width="500" :before-close="onClose">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button size="small" @click="onClose">{{ $t('common.cancel') }}</el-button>
          <el-button type="primary"  size="small" @click="onChangePwd" :loading="showPwdLoading">{{ $t('common.confirm') }}</el-button>
        </span>
      </template>
      <el-form :model="pwdForm" ref="pwdForm" label-width="100px" :rules="pwdRules" label-position="left" label-suffix=" : ">
        <el-form-item label="原始密码" prop="oldPwd" style="width: 100%;">
          <el-input v-model="pwdForm.oldPwd" type="password" show-password :maxlength="128"></el-input>
        </el-form-item>
        <el-form-item label="新密码" prop="firstPwd" style="width: 100%;">
          <el-input v-model="pwdForm.firstPwd" type="password" show-password :maxlength="128"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="secondPwd" style="width: 100%;">
          <el-input v-model="pwdForm.secondPwd" type="password" show-password :maxlength="128"></el-input>
        </el-form-item>
      </el-form>
    </re-dialog> 
  </div>
...
export default {
  ...
  data() {
    return{
      ...
      pwdRules: {
        ...
        firstPwd: [{required: true, message: this.$t('validate.required'), trigger: 'blur' }],
        secondPwd: [{
          required: true,
  ...
  methods: {
    ...
    getSystemConfig() {
      this.$api.get("/webapp/get_sysconfig?remote_ip=" + this.$route.query.ip).then((response) => {
        ...
        this.setTimering(response, "get_blk", (res) => {
          if (res.state == 0) {
            ...
            this.systemInfo.systemdisk.forEach(item => {
              ...
              // this.$nextTick(()=>{
              //   if (disk) this.$refs.blockData.toggleRowSelection(disk,true);
              // })
            })
          } else {
    ...
    onShowProver() {
      this.$api.get('/webapp/get_version?remote_ip=' + this.$route.query.ip).then((response) => {
        this.setTimering(response, "get_version", (res) => {
          if (res.state == 0) {
            this.$messageBox.alert(
              '<div class="prover"><div>'+ this.$t('common.provernum') + '：</div><div class="indent">'+ res.params.shortver.app_version +'</div><div class="indent">mwm1278_v2-storagemngweb-1.0.0</div></div>',
    ...
    onChangePwd() {
      this.$refs.pwdForm.validate((valid) => {
        if (valid) {
          this.$api.post('/webapp/edit_user_account', {
