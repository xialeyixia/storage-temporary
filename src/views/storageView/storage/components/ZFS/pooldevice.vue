  ...
  <div>
        ...
        <template #default="scope">
          <el-dropdown trigger="click" @command="moreOperate">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  :command="{ event: replaceDisk, row: scope.row }"
                  v-if="
                    !scope.row.children &&
                    scope.row.tag != 'spare' &&
                    scope.row.tag != 'cache'
                  "
                  >{{ $t('storage.replacedisk') }}</el-dropdown-item
                >
                <el-dropdown-item
                  :command="{ event: onlineDisk, row: scope.row }"
                  v-if="!scope.row.children && scope.row.tag != 'spare'"
                  >{{ $t('storage.online') }}</el-dropdown-item
                >
                <el-dropdown-item
                  :command="{ event: offlineDisk, row: scope.row }"
                  v-if="!scope.row.children && scope.row.tag != 'spare'"
                  >{{ $t('storage.offline') }}</el-dropdown-item
                >
                <el-dropdown-item
                  :command="{ event: addMirrorDisk, row: scope.row }"
                  v-if="
                    !scope.row.children &&
                    scope.row.level != 'spare' &&
                    scope.row.tag != 'spare' &&
                    scope.row.tag != 'cache'
                  "
                  >{{ $t('storage.addmirdisk') }}</el-dropdown-item
                >
                <el-dropdown-item
                  :command="{ event: removeMirrorDisk, row: scope.row }"
                  v-if="
                    !scope.row.children &&
                    (scope.row.level == 'mirror' || scope.row.level == 'spare')
                  "
                  >{{ $t('control.delete') }}</el-dropdown-item
                >
                <el-dropdown-item
                  :command="{ event: removeDisk, row: scope.row }"
                  v-if="
                    scope.row.children ||
                    !scope.row.children ||
                    scope.row.level != 'mirror' ||
                    scope.row.level != 'spare'
                  "
                  >{{ $t('control.delete') }}</el-dropdown-item
                >
    ...
    <re-dialog v-model="diskVisible" :width="430">
      <template #title>
        {{
          (visibleType == 0
            ? $t('storage.replacedisk')
            : $t('storage.addmirdisk')) +
          ' ( ' +
          diskForm.olddisk +
          ' )'
        }}
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            ...
            @click="sure"
            :loading="sureLoading"
            :disabled="sureLoading || !diskForm.device"
      ...
      <el-form :model="diskForm" ref="diskForm" label-position="left">
        ...
        >
          <el-table
            ...
            :data="tableFilter([...blockData, ...diskpartData])"
  ...
  methods: {
    ...
    tableFilter(arr) {
      ...
      if (!this.visibleType && this.currentRow.tag == 'data') {
        let spare = this.deviceData.filter(
          (item) => item.tag === 'spare' && item.state === 'AVAIL',
        )
        newArr = [...newArr, ...spare]
      }
      return newArr
    ...
    sure() {
      let type =
        this.visibleType == 0
          ? 'repalce_zfs_pool_device'
          : 'add_zfs_pool_mirror_disk'
      let querydata = {
      ...
      this.$api
        .post(
          '/webapp/' + type + '?remote_ip=' + this.$route.query.ip,
          querydata,
          true,
        )
        .then((res) => {
    ...
    removeDisk(row) {
      this.$messageBox
        .confirm(
          this.$t('storage.deldisk') +
            ' - ' +
            (row.disk ? row.disk : row.groupid),
        ...
        .then(() => {
          ...
          this.$api
            .deletes(
              '/webapp/delete_zfs_pool_disk?remote_ip=' + this.$route.query.ip,
              querydata,
              true,
            )
            .then((res) => {
    ...
    removeMirrorDisk(row) {
      this.$messageBox
        ...
        .then(() => {
          ...
          this.$api
            .deletes(
              '/webapp/delete_zfs_pool_mirror_disk?remote_ip=' +
                this.$route.query.ip,
              querydata,
              true,
            )
    ...
    sureOfflineDisk() {
      ...
      this.$api
        .put(
          '/webapp/offline_zfs_pool_disk?remote_ip=' + this.$route.query.ip,
          querydata,
          true,
        )
        .then((res) => {
    ...
    onlineDisk(row) {
      this.$messageBox
        ...
        .then(() => {
          ...
          this.$api
            .put(
              '/webapp/online_zfs_pool_disk?remote_ip=' + this.$route.query.ip,
              querydata,
              true,
            )
            .then((res) => {
