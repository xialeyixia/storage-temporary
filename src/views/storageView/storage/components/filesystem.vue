<template>
  <div class="storage-content">
    <el-button color="#579cdc" style="color: #fff;" type="primary" size="small" @click="mountFS">{{ $t('storage.mount') }}</el-button>
    <div class="maincontents">
      <el-table :data="fileSystemData" v-loading="spinning" table-layout="auto" header-cell-class-name="table-header-row" element-loading-text="Loading...">
        ...
        <el-table-column :label="$t('storage.availsize')" prop="avail" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column :label="$t('table.operate')" fixed="right">
          <template #default="scope">
            <el-button size="small" type="primary" @click="removeFileSystem(scope.row)" :disabled="scope.row.occupyname !== null">{{$t('storage.unmount')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <re-dialog v-model="mountFSVisible" :width="430">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button size="small" @click="mountFSVisible = false" :disabled="sureLoading">{{ $t('common.cancel') }}</el-button>
          <el-button type="primary"  size="small" @click="sureMountFS" :loading="sureLoading" :disabled="sureLoading || !mountFSForm.name">{{ $t('common.confirm') }}</el-button>
        </span>
      </template>
      <el-form :model="mountFSForm" ref="mountFSForm" label-width="120px" :rules="mountFSRules" label-position="left">
        <el-form-item :label="$t('storage.mounton')" style="width: 100%" prop="mounton">
          <el-input v-model.trim="mountFSForm.mounton" :maxlength="128"></el-input>
        </el-form-item>
        <el-form-item :label="$t('storage.fstype')" style="width: 100%" v-show="mountFSForm.type == 0">
          <el-select v-model="mountFSForm.fstype"  style="width: 100%">
            <el-option v-for="item in fstypeList" :key="item" :value="item" :label="item"></el-option>
          </el-select>
        </el-form-item>
        <div style="width: 100%">
          <div style="margin-bottom: 10px">{{$t('common.choosedevice')}}</div>
          <el-table v-loading="tableLoading" :data="tableFilter([...blockData,...diskpartData,...datasetData])" header-cell-class-name="table-header-row"
            element-loading-text="Loading..." highlight-current-row @current-change="handleCurrentChange" max-height="260px">
            <el-table-column width="40px">
              <template #default="scope">
                <el-radio v-model="mountFSForm.name" :label="scope.row.name"></el-radio>
              </template>
            </el-table-column>
        ...
        <div style="text-align: center; margin-top: 10px" v-show="mountFSForm.type == 0">
          <el-checkbox v-model="mountFSForm.automount" class="checkbox">{{$t('storage.automount')}}</el-checkbox>
          <el-checkbox v-model="mountFSForm.format" class="checkbox">{{$t('storage.formatfs')}}</el-checkbox>
          <div style="color: red">{{'(' + $t('storage.fsnote') + ' ! )'}}</div>
        </div>
      </el-form>
    </re-dialog>
  </div>
  ...
  methods:{
    ...
    removeFileSystem(row){
      this.$messageBox.confirm(
        ...
        this.spinning = true;
        this.$api.deletes("/webapp/delete_fs?remote_ip=" + this.$route.query.ip, { name: row.name, mounton: row.mounton, fstype: row.fstype }).then((response) => {
          this.setTimering(response, "delete_fs", (res) => {
            this.spinning = false;
            if (res.state == 0) {
              this.getFSlist();
    ...
    sureMountFS(){
      this.$refs.mountFSForm.validate(valid => {
        if (valid) {
          ...
          this.$api.post("/webapp/set_fs?remote_ip=" + this.$route.query.ip, querydata).then((response) => {
            this.setTimering(response, "set_fs", (res) => {
