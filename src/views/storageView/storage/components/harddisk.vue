  ...
  <div class="storage-content">
    <div class="maincontents">
        ...
        <el-table-column type="expand">
          <template #default="props">
            <div style="padding: 5px 20px">
              <el-table
                :data="props.row.dlist"
                table-layout="auto"
                header-cell-class-name="table-header-row"
              >
                <el-table-column
                  :label="$t('storage.partname')"
                  prop="name"
                  :formatter="formatvalue"
                ></el-table-column>
                <el-table-column
                  :label="$t('storage.number')"
                  prop="number"
                  :formatter="formatvalue"
                ></el-table-column>
                <el-table-column
                  :label="$t('storage.parttype')"
                  prop="partitiontype"
                  :formatter="formatvalue"
                ></el-table-column>
                <el-table-column
                  :label="$t('storage.disklabeltype')"
                  prop="disklabeltype"
                  :formatter="formatvalue"
                ></el-table-column>
                <el-table-column
                  :label="$t('storage.startcylinder')"
                  prop="start"
                  :formatter="formatvalue"
                ></el-table-column>
                <el-table-column
                  :label="$t('storage.endcylinder')"
                  prop="end"
                  :formatter="formatvalue"
                ></el-table-column>
                <el-table-column
                ...
                <el-table-column :label="$t('table.operate')" fixed="right">
                  <template #default="scope">
                    <el-button
                      ...
                      @click="removeDiskPart(scope.row)"
        ...
        <el-table-column :label="$t('table.operate')" fixed="right">
          <template #default="scope">
            <el-button
              size="small"
              type="primary"
              @click="createPartition(scope.row)"
              >{{ $t('storage.createpart') }}</el-button
            >
            <el-button size="small" type="primary" @click="viewsmart(scope.row)"
              >S.M.A.R.T</el-button
            >
          </template>
        </el-table-column>
        ...
        <el-form :model="addDiskPartForm" ref="addDiskPartForm">
          ...
          <span
            style="
              ...
              width: 100%;
            "
            >* 修改磁盘标签类型将导致原有分区被删除</span
          >
          <el-form-item
            :label="$t('storage.parttype')"
            v-show="addDiskPartForm.disklabeltype == 'mbr'"
            style="width: 100%"
          >
            <el-select
              v-model="addDiskPartForm.partitiontype"
              style="width: 100%"
            >
              <el-option
      ...
      <re-dialog v-model="smart.visible" :width="1000">
        <template #title>{{
          'S.M.A.R.T.' + ' ( ' + smart.currentdisk.name + ' )'
        }}</template>
        <el-tabs v-model="activeName" type="card" @tab-change="handleClick">
          <el-tab-pane
            :label="$t('storage.attributes')"
            name="attributes"
            v-if="smart.currentdisk.SMARTopen"
          ></el-tab-pane>
          <el-tab-pane
            :label="$t('storage.errlog')"
            name="errlog"
            v-if="smart.currentdisk.SMARTopen"
          ></el-tab-pane>
          <el-tab-pane
            :label="$t('storage.selftest')"
            name="selftest"
            v-if="
              smart.currentdisk.SMARTopen &&
              !smart.currentdisk.model.match('nvme')
            "
          ></el-tab-pane>
          <el-tab-pane
            :label="$t('storage.extendedinfo')"
            name="info"
          ></el-tab-pane>
        </el-tabs>
        <div v-show="activeName == 'attributes'" class="smart-content">
          <el-form>
            <el-form-item :label="$t('storage.healthstatus')">{{
              smart.healthstatus
            }}</el-form-item>
          </el-form>
          ...
          >
            ...
            ></el-table-column>
            <el-table-column
              :label="$t('storage.currentvalue')"
              prop="value"
              :formatter="formatvalue"
            ></el-table-column>
            <el-table-column
              :label="$t('storage.worstvalue')"
              prop="worst"
              :formatter="formatvalue"
            ></el-table-column>
            <el-table-column
              :label="$t('storage.threshold')"
              prop="thresh"
              :formatter="formatvalue"
            ></el-table-column>
            <el-table-column
            ...
            ></el-table-column>
            <el-table-column
              :label="$t('storage.updatemode')"
              prop="update"
              :formatter="formatvalue"
            ></el-table-column>
            <el-table-column
              :label="$t('storage.failuretime')"
              prop="whenfailed"
              :formatter="formatvalue"
            ></el-table-column>
            <el-table-column
              :label="$t('storage.rawvalue')"
              prop="rawvalue"
              :formatter="formatvalue"
            ></el-table-column>
          </el-table>
          ...
          <el-form>
            <el-form-item :label="$t('storage.selftesttype')">
              <el-select v-model="smart.testtype" style="width: 100%">
                <el-option
                  :label="$t('storage.shortselftest')"
                  value="short"
                ></el-option>
                <el-option
                  :label="$t('storage.longselftest')"
                  value="long"
                ></el-option>
              </el-select>
            </el-form-item>
        ...
        <template #footer>
          <span class="dialog-footer">
            <el-button
              size="small"
              type="primary"
              v-show="activeName == 'selftest'"
              @click="startSelfTest"
              :disabled="sureLoading || smart.teststatus"
              :loading="smart.teststatus"
      ...
      <re-dialog v-model="systemsettings.visible" :width="1000">
        ...
        <template #footer>
          <span class="dialog-footer">
            ...
            <el-button
              size="small"
              @click="systemsettings.visible = false"
              :disabled="sureLoading"
              >{{ $t('common.close') }}</el-button
            >
          </span>
...
export default {
  ...
  methods: {
    ...
    getDiskDeviceList() {
      ...
      this.$api
        .get(
          '/webapp/get_diskdevice?remote_ip=' + this.$route.query.ip,
          {},
          true,
        )
        .then((res) => {
    ...
    sureCreatePartition() {
      this.$api
        .post(
          '/webapp/create_diskpart?remote_ip=' + this.$route.query.ip,
          this.addDiskPartForm,
          true,
        )
        .then((res) => {
    ...
    removeDiskPart(row) {
      this.$messageBox
        ...
        .then(() => {
          ...
          this.$api
            .deletes(
              '/webapp/delete_diskpart?remote_ip=' + this.$route.query.ip,
              { name: row.name },
              true,
            )
            .then((res) => {
    ...
    viewsmart(row) {
      ...
      this.$api
        .get(
          '/webapp/get_disk_smart_support_info?remote_ip=' +
            this.$route.query.ip,
          { name: row.name },
          true,
        )
    ...
    getSmartAttr() {
      ...
      this.$api
        .get(
          '/webapp/get_disk_smart_health_status?remote_ip=' +
            this.$route.query.ip,
          querydata,
          true,
        )
        .then((res) => {
          ...
          this.$api
            .get(
              '/webapp/get_disk_smart_attributes?remote_ip=' +
                this.$route.query.ip,
              querydata,
              true,
            )
    ...
    getSmartInfo() {
      this.$api
        .get(
          '/webapp/get_disk_smart_log?remote_ip=' + this.$route.query.ip,
          {
            name: this.smart.currentdisk.name,
          },
          true,
    ...
    getSmartErrLog() {
      this.$api
        .get(
          '/webapp/get_disk_smart_errorlog?remote_ip=' + this.$route.query.ip,
          { name: this.smart.currentdisk.name },
          true,
        )
        .then((res) => {
    ...
    changeSmartStatus(name, val) {
      ...
      this.$api
        .post(
          '/webapp/control_disk_smart?remote_ip=' + this.$route.query.ip,
          querydata,
          true,
        )
        .then((res) => {
    ...
    startSelfTest() {
      this.$api
        .post(
          '/webapp/start_disk_smart_selftest?remote_ip=' + this.$route.query.ip,
          { name: this.smart.currentdisk.name, type: this.smart.testtype },
          true,
        )
        .then((res) => {
          if (res.state == 0) {
    ...
    stopSelfTest() {
      this.$api
        .post(
          '/webapp/stop_disk_smart_selftest?remote_ip=' + this.$route.query.ip,
          { name: this.smart.currentdisk.name },
          ture,
        )
        .then((res) => {
    ...
    getSelfTestProgress() {
      this.$api
        .get(
          '/webapp/get_disk_smart_selftest_progress?remote_ip=' +
            this.$route.query.ip,
          { name: this.smart.currentdisk.name },
          true,
        )
    ...
    getSelfTestResult() {
      this.$api
        .get(
          '/webapp/get_disk_smart_selftest_result?remote_ip=' +
            this.$route.query.ip,
          { name: this.smart.currentdisk.name },
          true,
        )
